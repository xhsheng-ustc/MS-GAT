...
def eval(ground_truth_file_list, degraded_file_list,weight_file_list, batch_size):
  distortions = 0. 
  removal_u_psnr_sum = 0.
  degraded_u_psnr_sum = 0.
  # removal_u_psnr_sum = 0.
  # degraded_u_psnr_sum = 0.
  ...
  for i in range(len(ground_truth_file_list)//batch_size):
    ...
    eval_x_degraded_attribute_u = eval_x_degraded_attribute[:,:,1]
    eval_x_degraded_attribute_u = tf.expand_dims(eval_x_degraded_attribute_u,axis=-1) 
    ...
    eval_x_removal_attribute = analysis_transform(eval_x_degraded_coordinate,eval_x_degraded_attribute_u,eval_x_weight)

    u_loss_degraded = tf.cast(tf.reduce_mean(tf.square(eval_x_attribute[:,:,1]-eval_x_degraded_attribute[:,:,1])),'float32')

    u_loss_removal = tf.cast(tf.reduce_mean(tf.square(eval_x_attribute[:,:,1]-eval_x_removal_attribute[:,:,0])),'float32')

    u_loss_removal_sum = tf.cast(tf.reduce_sum(tf.square(eval_x_attribute[:,:,1]-eval_x_removal_attribute[:,:,0])),'float32')

    color_loss=u_loss_removal_sum

    removal_u_psnr = float(10.0 * math.log(1/u_loss_removal, 10))
    degraded_u_psnr = float(10.0 * math.log(1/u_loss_degraded, 10)) 
    
    distortion = color_loss
    distortions = distortions + distortion

    removal_u_psnr_sum = removal_u_psnr_sum + removal_u_psnr
    degraded_u_psnr_sum = degraded_u_psnr_sum + degraded_u_psnr


  return distortions/(i+1),removal_u_psnr_sum/(i+1), degraded_u_psnr_sum/(i+1)
...
def train():
  ...
  for step in range(int(global_step), int(NUM_ITEATION+1)):
    ...
    x_degraded_attribute_u = x_degraded_attribute[:,:,1]
    x_degraded_attribute_u = tf.expand_dims(x_degraded_attribute_u,axis=-1) 
    ...
    with tf.GradientTape() as model_tape:
      x_color_removal = analysis_transform(x_degraded_coordinate,x_degraded_attribute_u,x_degraded_weight)

      # losses.
      # We calculate reduce_mean for calculating psnr
      u_loss_degraded = tf.cast(tf.reduce_mean(tf.square(x_ground_truth_attribute[:,:,1]-x_degraded_attribute[:,:,1])),'float32')

      u_loss_removal = tf.cast(tf.reduce_mean(tf.square(x_ground_truth_attribute[:,:,1]-x_color_removal[:,:,0])),'float32')

      u_loss_removal_sum = tf.cast(tf.reduce_sum(tf.square(x_ground_truth_attribute[:,:,1]-x_color_removal[:,:,0])),'float32')
    ...
    removal_u_psnr_sum += removal_u_psnr
    degraded_u_psnr_sum += degraded_u_psnr
    num += 1

    # Display.
    if (step + 1) % DISPLAY_STEP == 0:
      train_loss_sum  /= num
      removal_u_psnr_sum /= num
      degraded_u_psnr_sum /= num

      print('Iter:{:},Loss:{:.4f}, u_psnr ={:.4f}, degraded_u_psnr = {:.4f}'.format(step+1,train_loss_sum,removal_u_psnr_sum,degraded_u_psnr_sum))
      print()
      ...
      with writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(1):
        ...
        tf.contrib.summary.scalar('removal_u_psnr_sum',removal_u_psnr_sum)      
    ...
    if (step + 1) % EVAL_STEP == 0:
      print('evaluating...')
      eval_Distortion,eval_removal_u_psnr, eval_degraded_u_psnr = eval(eval_ground_truth_file_list, eval_degraded_file_list,eval_weight_file_list,batch_size=BATCH_SIZE)
      print("Distortion={:.4f},val_u_psnr ={:.4f}, val_degraded_u_psnr = {:.4f}".format(eval_Distortion,eval_removal_u_psnr, eval_degraded_u_psnr))
      print()
      ...
      with eval_writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(1):
        ...
        tf.contrib.summary.scalar('eval_removal_u_psnr', eval_removal_u_psnr)
...
# checkpoint.
checkpoint_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/checkpoints/QP{}/u/'.format(QP))
...
log_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/logs/QP{}/u/'.format(QP))

eval_log_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/logs/eval/QP{}/u/'.format(QP))
