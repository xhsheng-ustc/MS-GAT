...
degraded_file_list = sorted(glob.glob('/gdata/shengxh/TMC13v12_split_h5_with_weight/QP'+str(args.QP)+'/*.h5'))
ground_truth_file_list = sorted(glob.glob('/gdata/shengxh/TMC13v12_split_h5_with_weight/Lossless'+'/*.h5'))
weight_file_list = sorted(glob.glob('/gdata/shengxh/TMC13v12_split_h5_with_weight/weight'+'/*.h5'))
...
print(eval_ground_truth_file_list)
...
def eval(ground_truth_file_list, degraded_file_list,weight_file_list, batch_size):
  distortions = 0. 
  removal_y_psnr_sum = 0.
  degraded_y_psnr_sum = 0.
  ...
  for i in range(len(ground_truth_file_list)//batch_size):
    ...
    eval_x_degraded_attribute_y = eval_x_degraded_attribute[:,:,0]
    eval_x_degraded_attribute_y = tf.expand_dims(eval_x_degraded_attribute_y,axis=-1) 
    ...
    eval_x_removal_attribute = analysis_transform(eval_x_degraded_coordinate,eval_x_degraded_attribute_y,eval_x_weight)

    y_loss_degraded = tf.cast(tf.reduce_mean(tf.square(eval_x_attribute[:,:,0]-eval_x_degraded_attribute[:,:,0])),'float32')

    y_loss_removal = tf.cast(tf.reduce_mean(tf.square(eval_x_attribute[:,:,0]-eval_x_removal_attribute[:,:,0])),'float32')

    y_loss_removal_sum = tf.cast(tf.reduce_sum(tf.square(eval_x_attribute[:,:,0]-eval_x_removal_attribute[:,:,0])),'float32')

    color_loss=y_loss_removal_sum

    removal_y_psnr = float(10.0 * math.log(1/y_loss_removal, 10))
    degraded_y_psnr = float(10.0 * math.log(1/y_loss_degraded, 10)) 
    
    distortion = color_loss
    distortions = distortions + distortion

    removal_y_psnr_sum = removal_y_psnr_sum + removal_y_psnr
    degraded_y_psnr_sum = degraded_y_psnr_sum + degraded_y_psnr


  return distortions/(i+1),removal_y_psnr_sum/(i+1), degraded_y_psnr_sum/(i+1)
...
def train():
  ...
  for step in range(int(global_step), int(NUM_ITEATION+1)):
    ...
    x_degraded_attribute_y = x_degraded_attribute[:,:,0]
    x_degraded_attribute_y = tf.expand_dims(x_degraded_attribute_y,axis=-1) 
    ...
    with tf.GradientTape() as model_tape:
      x_color_removal = analysis_transform(x_degraded_coordinate,x_degraded_attribute_y,x_degraded_weight)

      # losses.
      to_yuv_color = x_ground_truth_attribute
      to_yuv_color_degraded = x_degraded_attribute
      to_yuv_color_removal = x_color_removal
      # We calculate reduce_mean for calculating psnr
      y_loss_degraded = tf.cast(tf.reduce_mean(tf.square(to_yuv_color[:,:,0]-to_yuv_color_degraded[:,:,0])),'float32')

      y_loss_removal = tf.cast(tf.reduce_mean(tf.square(to_yuv_color[:,:,0]-to_yuv_color_removal[:,:,0])),'float32')

      y_loss_removal_sum = tf.cast(tf.reduce_sum(tf.square(to_yuv_color[:,:,0]-to_yuv_color_removal[:,:,0])),'float32')
    ...
    removal_y_psnr_sum += removal_y_psnr
    degraded_y_psnr_sum += degraded_y_psnr
    num += 1

    # Display.
    if (step + 1) % DISPLAY_STEP == 0:
      train_loss_sum  /= num
      removal_y_psnr_sum /= num
      degraded_y_psnr_sum /= num

      print('Iter:{:},Loss:{:.4f}, y_psnr ={:.4f}, degraded_y_psnr = {:.4f}'.format(step+1,train_loss_sum,removal_y_psnr_sum,degraded_y_psnr_sum))
      print()
      ...
      with writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(1):
        ...
        tf.contrib.summary.scalar('removal_y_psnr_sum',removal_y_psnr_sum)      
    ...
    if (step + 1) % EVAL_STEP == 0:
      print('evaluating...')
      eval_Distortion,eval_removal_y_psnr, eval_degraded_y_psnr = eval(eval_ground_truth_file_list, eval_degraded_file_list,eval_weight_file_list,batch_size=BATCH_SIZE)
      print("Distortion={:.4f},val_y_psnr ={:.4f}, val_degraded_y_psnr = {:.4f}".format(eval_Distortion,eval_removal_y_psnr, eval_degraded_y_psnr))
      print()
      ...
      with eval_writer.as_default(), tf.contrib.summary.record_summaries_every_n_global_steps(1):
        ...
        tf.contrib.summary.scalar('eval_removal_y_psnr', eval_removal_y_psnr)
...
# checkpoint.
checkpoint_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/checkpoints/QP{}/y/'.format(QP))
...
log_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/logs/QP{}/y/'.format(QP))

eval_log_dir = os.path.join('/gdata/shengxh/MS-GAT2/code/main/logs/eval/QP{}/y/'.format(QP))
